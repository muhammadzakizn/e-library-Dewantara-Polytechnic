...
import { Mail, Lock, Eye, EyeOff, ArrowRight, AlertCircle, User, GraduationCap, ArrowLeft, Loader2, Home, CheckCircle2, MailCheck, Shield, BookOpen } from 'lucide-react';
...
export default function LoginPage() {
    ...
    const [loginMode, setLoginMode] = useState<'mahasiswa' | 'admin'>('mahasiswa');
    ...
    const handleAdminLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let loginEmail = adminIdentifier;
            ...
            if (profile?.role !== 'admin' && profile?.role !== 'dosen') {
                ...
                throw new Error('Akses ditolak. Akun ini bukan administrator atau dosen.');
            }
        ...
        <div className="min-h-screen flex bg-white dark:bg-gray-950 overflow-hidden">
            ...
            <div className="hidden lg:flex lg:w-1/2 p-2 relative transition-all duration-700 ease-in-out">
                <div className="relative overflow-hidden rounded-3xl w-full h-full">
                    ...
                    <div className={`absolute inset-0 mix-blend-multiply transition-colors duration-500 ${loginMode === 'admin' ? 'bg-gradient-to-r from-gray-900/85 to-slate-800/85' : 'bg-gradient-to-r from-primary-900/80 to-primary-800/80'}`} />

                    <div className="relative z-10 flex flex-col justify-between p-12 h-full text-white">
                        <div className="flex items-center gap-3">
                            <img src="/logo-poltek.png" alt="Logo" className="w-10 h-10 object-contain" />
                            <div>
                        ...
                        <div className="max-w-md">
                            ...
                            <p className="text-lg opacity-80 leading-relaxed">
                                {loginMode === 'admin'
                                    ? 'Kelola perpustakaan digital, moderasi konten, dan pantau aktivitas akademik.'
            ...
            <div className={`w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 transition-all duration-500`}>
                <div className="w-full max-w-md space-y-6">
                    ...
                    {/* Back to Home */}
                    <Link href="/" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-primary-500 transition-colors">
                    ...
                    <div className="bg-gray-100 dark:bg-gray-900 p-1 rounded-xl flex">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-medium transition-all duration-200 ${loginMode === 'mahasiswa'
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-medium transition-all duration-200 ${loginMode === 'admin'
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="text-center lg:text-left">
                                ...
                                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    Login dengan akun administrator atau dosen.
                                </p>
                            </div>
                            ...
                            <form onSubmit={handleAdminLogin} className="space-y-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            placeholder="admin@polidewa.ac.id atau username"
                                ...
                                <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-3 rounded-xl transition-all shadow-lg shadow-blue-600/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                                    {isLoading ? <Loader2 className="animate-spin w-5 h-5" /> : <>Login ke Dashboard <ArrowRight className="w-5 h-5" /></>}
                                </button>
                            </form>
                            ...
                            <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl p-4 text-center">
                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                    🔐 Protected System. Hanya akun dengan role <strong>admin</strong> atau <strong>dosen</strong> yang bisa mengakses dashboard.
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        <>
                            ...
                            <div className="text-center lg:text-left">
                                <div className="lg:hidden flex justify-center mb-6">
                                    <img src="/logo-poltek.png" alt="Logo Politeknik Dewantara" className="w-12 h-12 object-contain" />
                                </div>
                                <h2 className="text-3xl font-bold text-gray-900 dark:text-white">
                                    {step === 'email' ? 'Selamat Datang 👋' : step === 'login' ? 'Selamat Datang Kembali!' : step === 'register' ? 'Buat Akun Baru' : ''}
                                </h2>
                                <p className="mt-2 text-gray-500 dark:text-gray-400">
                                    {step === 'email' ? 'Masuk atau daftar untuk melanjutkan akses perpustakaan.'
                                        : step === 'login' ? `Masuk sebagai ${email}`
                                            : step === 'register' ? 'Lengkapi data diri Anda untuk mendaftar.' : ''}
                                </p>
                            </div>
                            ...
                            {step === 'login' && (
                                <form onSubmit={handleLogin} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                    ...
                                    <button type="button" onClick={() => setStep('email')} className="w-full text-center text-gray-500 text-sm hover:text-gray-800 dark:hover:text-gray-200 transition-colors">
                                        ← Kembali
                                    </button>
                                </form>
                            )}
                            ...
                            {step === 'register' && (
                                <form onSubmit={handleRegister} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                                    ...
                                    <button type="button" onClick={() => setStep('email')} className="w-full text-center text-gray-500 text-sm hover:text-gray-800 transition-colors">
                                        ← Gunakan email lain
                                    </button>
                                </form>
                            )}
                            ...
                            {step === 'success' && (
                                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 text-center space-y-6">
                                    ...
                                    <div className="relative mx-auto w-24 h-24">
                                        ...
                                        <div className="relative w-24 h-24 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center shadow-lg shadow-green-500/25">
                                    ...
                                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-2xl p-6 space-y-4">
                                        <div className="w-14 h-14 bg-blue-100 dark:bg-blue-800/50 rounded-xl flex items-center justify-center mx-auto">
                                            <MailCheck className="w-7 h-7 text-blue-600 dark:text-blue-400" />
                                        </div>
                                        <div>
                                            <h3 className="font-semibold text-blue-900 dark:text-blue-100 mb-1">Verifikasi Email Anda</h3>
                                            <p className="text-sm text-blue-700 dark:text-blue-300">
                                                Kami telah mengirimkan link verifikasi ke:
                                            </p>
                                            <p className="font-medium text-blue-900 dark:text-blue-100 mt-1 bg-blue-100 dark:bg-blue-800/50 px-3 py-1.5 rounded-lg inline-block text-sm">
                                                {email}
                                            </p>
                                        </div>
                                        <div className="text-left space-y-2 text-sm text-blue-700 dark:text-blue-300">
                                            <p className="flex items-start gap-2">
                                                <span className="bg-blue-200 dark:bg-blue-700 text-blue-800 dark:text-blue-200 rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">1</span>
                                                Buka email Anda
                                            </p>
                                            <p className="flex items-start gap-2">
                                                <span className="bg-blue-200 dark:bg-blue-700 text-blue-800 dark:text-blue-200 rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">2</span>
                                                Klik link <strong>&quot;Confirm your mail&quot;</strong>
                                            </p>
                                            <p className="flex items-start gap-2">
                                                <span className="bg-blue-200 dark:bg-blue-700 text-blue-800 dark:text-blue-200 rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">3</span>
                                                Kembali dan login dengan akun baru
                                            </p>
                                        </div>
                                    </div>

                                    {/* Tip */}
                                    <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-xl p-3 flex items-start gap-3">
                                        <span className="text-lg">💡</span>
                                        <p className="text-xs text-yellow-700 dark:text-yellow-300 text-left">
                                            Tidak menerima email? Cek folder <strong>Spam</strong> atau <strong>Promosi</strong> di email Anda.
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <button
                                            onClick={() => { setStep('email'); setEmail(''); setPassword(''); setName(''); setNim(''); setProdi(''); }}
                                            className="w-full text-center text-gray-500 text-sm hover:text-gray-800 dark:hover:text-gray-200 transition-colors"
                                        >
                                            Daftar akun lain
                                        </button>
                                    </div>
